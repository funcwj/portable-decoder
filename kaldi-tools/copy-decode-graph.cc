...
struct SimpleArc {
  ...
  explicit SimpleArc(const fst::StdArc &arc)
      : ilabel(arc.ilabel),
        olabel(arc.olabel),
...
int main(int argc, char const *argv[]) {
  try {
    ...
    const char *usage =
        "Transform format of Kaldi's decode graph(HCLG.fst) which is supported "
        "by simple asr-decoder\n"
        "\n"
    ...
    KALDI_LOG << "Read decoder graph(type:" << decode_fst->Type()
              << ") contains " << num_states << " states and " << num_arcs
              << " arcs with start index " << decode_fst->Start();
