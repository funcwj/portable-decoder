...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "which aims for propagating through pytorch(support few component "
        "now).\n"
        "\n"
    ...
    for (int32 i = 0; i < num_components; i++) {
      ...
      if (type == "FixedAffineComponent") {
        ...
        const FixedAffineComponent *fa =
            dynamic_cast<const FixedAffineComponent *>(c);
        ...
        WriteToken(os, true, "Linear");
        const NaturalGradientAffineComponent *ng =
            dynamic_cast<const NaturalGradientAffineComponent *>(c);
        ...
        const BatchNormComponent *bn =
            dynamic_cast<const BatchNormComponent *>(c);
