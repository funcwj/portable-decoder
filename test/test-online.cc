...
void TestOnlineSplitter() {
  ...
  for (Int32 i = 0; i < N / C; i++) {
    ...
    splitter.Frame(egs + i * C, C, online_frames.data(),
                   online_frames.stride());
    std::cout << "Online " << num_frames << " frames: " << std::endl
              << online_frames << std::endl;
  }
  splitter.Reset();
  ...
  std::cout << "Offline " << num_frames << " frames: " << std::endl
            << offline_frames << std::endl;
  delete[] egs;
}
...
void TestExtractor() {
  ...
  Int32 num_frames = extractor.NumFrames(num_samples),
        dim = extractor.FeatureDim();
