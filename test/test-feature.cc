...
// echo "egs egs.wav" | compute-mfcc-feats --window-type=hamming --dither=0.0
// scp:-  ark,t:-
...
// Compared with command:
// echo "egs egs.wav" | compute-fbank-feats --window-type=hamming --dither=0.0
// --num-mel-bins=40 scp:-  ark,t:-
...
void TestFBank() {
  ...
  LOG_INFO << fbank.rows() << " x " << fbank.cols() << "(" << fbank.stride()
           << ")";
  ComputeFeature(&computer, egs.Data(), num_samples, fbank.data(),
                 fbank.stride());
...
// Compared with command:
// echo "egs egs.wav" | compute-spectrogram-feats --window-type=hamming
// --dither=0.0 scp:-  ark,t:-

void TestSpectrogram() {
  SpectrogramOpts spectrogram_opts;
  std::cout << "Spectrogram configure: " << std::endl
            << spectrogram_opts.Configure();
  ...
  LOG_INFO << spectrogram.rows() << " x " << spectrogram.cols() << "("
           << spectrogram.stride() << ")";
  ComputeFeature(&computer, egs.Data(), num_samples, spectrogram.data(),
                 spectrogram.stride());
...
void TestFrameSplitter() {
  ...
  std::cout << "FrameSplitter configure: " << std::endl
            << frame_opts.Configure();
  FrameSplitter splitter(frame_opts);
  Wave egs;
  ReadWave("egs.wav", &egs);

  Int32 num_samples = egs.NumSamples();
  const Int32 num_frames = splitter.NumFrames(num_samples),
              dim = splitter.FrameLength();
