...
void FFTComputer::ComplexBitReverse(Float32 *cplx_values, Int32 num_values) {
    ...
    for(Int32 j = 0, i = 0; i < num_values - 1; i++) {
        if(i < j) {
            std::swap(REAL_PART(cplx_values, i), 
                REAL_PART(cplx_values, j));
            std::swap(IMAG_PART(cplx_values, i), 
                IMAG_PART(cplx_values, j));
        }
...
void FFTComputer::ComplexFFT(Float32 *cplx_values, Int32 num_samples, bool invert)
{
    ...
    while(m < n) {
        ...
        while(cnt < inc) {
            ...
            for(int t = 0; t < m; t++, i++) {
                ...
                WR = cos_table_[k * s], WI = (invert ? -sin_table_[k * s]: sin_table_[k * s]);
