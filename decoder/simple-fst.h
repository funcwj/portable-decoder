...
class SimpleFst {
 public:
  ...
  UInt64 NumInputEpsilons(StateId state) const {
    return GetState(state)->NumInputEps();
  }

  UInt64 NumOutputEpsilons(StateId state) const {
    return GetState(state)->NumOutputEps();
  }
...
class StateIterator {
 public:
  StateIterator(const SimpleFst &fst)
      : cur_state_(0), num_states_(fst.NumStates()) {}
...
class ArcIterator {
 public:
  ArcIterator(const SimpleFst &fst, StateId state)
      : cur_arc_(0),
        num_arcs_(fst.NumArcs(state)),
        arc_ptr_(fst.GetState(state)->Arcs()) {}
