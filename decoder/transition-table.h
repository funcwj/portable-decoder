...
class TransitionTable {
 public:
  ...
  TransitionTable(const TransitionTable &table)
      : num_tids_(table.NumTransitionIds()), num_pdfs_(table.NumPdfs()) {
  ...
  void Read(std::istream &is) {
    ...
    LOG_INFO << "Read " << num_tids_ << " TransitionIds and " << num_pdfs_
             << " Pdfs";
    table_ = new Int32[num_tids_];
  ...
  void Write(std::ostream &os) {
    ...
    WriteBinary(os, reinterpret_cast<const char *>(table_),
                sizeof(Int32) * num_tids_);
  }

  Int32 TransitionIdToPdf(Int32 tid) {
    ASSERT(tid >= 1 && "TransitionId could not be negative");
    if (tid > num_tids_) {
      LOG_FAIL << "Index out of total number of transition ids, " << tid << "/"
               << num_tids_;
    }
    return table_[tid - 1];
  }
