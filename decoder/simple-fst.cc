...
Bool SimpleFst::Equal(const SimpleFst &fst) {
  if (fst.Start() != this->Start() || fst.NumStates() != this->NumStates()) {
    LOG_WARN << "Check start/num_states failed: " << fst.Start() << "/"
             << fst.NumStates() << " vs " << this->Start() << "/"
             << this->NumStates();
    return false;
  }
  for (StateIterator siter(fst); !siter.Done(); siter.Next()) {
    ...
    if (fst.NumArcs(state_id) != this->NumArcs(state_id) ||
      ...
      LOG_WARN << "Check " << state_id << "-th state's "
               << " num_arcs/final failed: " << fst.NumArcs(state_id) << "/"
               << fst.Final(state_id) << " vs " << this->NumArcs(state_id)
               << "/" << this->Final(state_id);
      return false;
    }
    for (ArcIterator other_aiter(fst, state_id), this_aiter(*this, state_id);
         !other_aiter.Done(); other_aiter.Next(), this_aiter.Next()) {
      const Arc &other_arc = other_aiter.Value(),
                &this_arc = this_aiter.Value();
...
void SimpleFst::Read(std::istream &is) {
  ...
  if (check_num_arcs != num_arcs)
    LOG_FAIL << "Check number of arcs failed, " << check_num_arcs << " vs "
             << num_arcs;
}
...
void SimpleFst::Write(std::ostream &os) {
  ...
  for (StateIterator siter(*this); !siter.Done(); siter.Next()) {
    ...
    for (ArcIterator aiter(*this, state); !aiter.Done(); aiter.Next()) {
      const Arc &arc = aiter.Value();
  ...
  if (check_num_states != NumStates())
    LOG_FAIL
        << "Number of state in current FST do not equal with values return "
