...
void ConfigureParser::LoadConfigure() {
    while (!conf_stream_.eof()) {
        ...
        if (space_iter != line.end() || equal_iter == line.end() || line.substr(0, 2) != "--")
            LOG_FAIL << "Wrong configure line: \'" << line << "\'";
        if (std::count(line.begin(), line.end(), '=') != 1)
            LOG_FAIL << "Wrong configure line: \'" << line << "\'";
        Int32 point_pos = line.find_first_of("."), equal_pos = line.find_first_of("=");
...
void ConfigureParser::AddOptions(const std::string &opt, const std::string &name, Float32 *value) {
    if (!table_.count(opt)) 
        return;
