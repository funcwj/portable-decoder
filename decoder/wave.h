...
// https://github.com/funcwj/asr-utils/blob/master/wave/wave.h
...
struct WaveHeader {
  ...
  UInt32 format_size;      // format header total bit size exclude format_id &
                           // format_size
...
class Wave {
 public:
  ...
  Wave(Float32 *data, Int32 num_samples, Int32 num_channels = 1,
    ...
    byte_per_sample_ = 2;
    header_ =
        GenWavHeader(num_samples, num_channels, sample_rate, byte_per_sample_);
    hold_memory_ = false;
  }

  ~Wave() {
    if (data_ && hold_memory_) delete[] data_;
  }
