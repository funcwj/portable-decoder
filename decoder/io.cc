...
void ReadBinaryBasicType(std::istream &is, T *t) {
  ...
  if (abs(num_bytes) != sizeof(*t))
    LOG_FAIL << "Expect " << sizeof(*t) << " bytes, but get "
             << static_cast<Int32>(num_bytes);
...
bool IsToken(const char *token) {
  if (token == NULL || *token == END_OF_STRING) return false;
...
template void ReadBinaryBasicType<Int32>(std::istream &is, Int32 *t);

template void ReadBinaryBasicType<Int64>(std::istream &is, Int64 *t);

template void ReadBinaryBasicType<UInt64>(std::istream &is, UInt64 *t);

template void ReadBinaryBasicType<Float32>(std::istream &is, Float32 *t);

template void WriteBinaryBasicType<Int32>(std::ostream &os, Int32 t);

template void WriteBinaryBasicType<Int64>(std::ostream &os, Int64 t);

template void WriteBinaryBasicType<UInt64>(std::ostream &os, UInt64 t);

template void WriteBinaryBasicType<Float32>(std::ostream &os, Float32 t);
